...
export function generateSkitTypePrompt(skit: SkitData, stage: Stage, continuing: boolean): string {
    ...
    switch (skit.type) {
        case SkitType.BEGINNING:
            return !continuing ?
                `This scene introduces the beginning of the story and establishes WHO THE PLAYER CHARACTER IS before any summoning happens. The player, ${stage.getSave().player.name}, is an ordinary person living an ordinary modern life. ` +
                `Open on a grounded, mundane moment in ${stage.getSave().player.name}'s day-to-day existence - establish their world, their routine, and their place in it, drawing on ${stage.getSave().player.name}'s profile. If the profile suggests a specific life, job, or setting, use it to inform where and how this opening moment plays out. If the profile gives no clear starting point, default to a neutral setting: ${stage.getSave().player.name} at home, alone, going about an ordinary evening. No supernatural events have ever touched this world, as far as anyone knows. ` +
                `During this scene, ${stage.getSave().player.name} notices an app on their phone they don't remember installing: a barebones, almost abandoned-looking gacha game. It is unremarkable at first glance - cheap art, a single glowing "summon" button, no ads, no studio name. ` +
                `The scene should let ${stage.getSave().player.name} discover and poke at this strange app, conveying a creeping sense that something is subtly wrong or uncanny about it, without yet revealing the full truth (that it summons real people, bound to them, as part of a hidden game). ` +
                `Keep this scene focused on the player, their mundane modern setting, and the unsettling arrival of the app - do NOT introduce any specific named summon or companion character here; incidental background people (a barista, a coworker, a passerby) may appear only as flavor. The actual first summoning is handled by a separate game mechanic, so this scene must end before anyone is summoned.` :
                `Continue this introductory scene. Keep the focus on ${stage.getSave().player.name}, their ordinary modern life, and the strange app they've just discovered on their phone - a barebones gacha game that feels subtly, uncannily off. ` +
                `Deepen the mundane, grounded setting and ${stage.getSave().player.name}'s place in it, and let the unease around the app build, without yet spelling out that it summons real people. Do not introduce a specific named summon; the first summoning is a separate mechanic. ` +
                `Once the setup is established, use a "[SUMMARY]" tag to summarize the events of this intro and end the scene before any summoning occurs.`;
        case SkitType.INTRO_CHARACTER:
            return !continuing ? 
                `This scene introduces a new summon, ${actor.name}, who has just been pulled out of their own life and into the player's ordinary modern world by the app, bound to the Summoner without their consent and with no clear way back. ${actor.name} is disoriented and may not understand what has happened to them. Establish their personality, their reaction to being summoned, and possibly some motivations.` :
...
export function buildSkitPrompt(skit: SkitData, stage: Stage, historyLength: number, instruction: string): string {
    ...
    let fullPrompt = `{{messages}}` +
        buildPromptSegment('Premise', `This is a modern-day visual novel game about a strange smartphone app that summons real people into the real world. ` +
        `The player character, ${playerName}, discovered an unremarkable-looking gacha app on their phone - but its "characters" are not fictional. Each summon pulls a living person out of their own life and into ${playerName}'s, bound to them as a summon, as part of an overarching game run by a mysterious, unseen Game Master. ` +
    ...
    while (retries > 0) {
        try {
            const fullPrompt = buildSkitPrompt(skit, stage, 5 + retries * 5, // Start with lots of history, reducing each iteration.
                ...
                `Generally, focus upon interpersonal dynamics, character growth, the bond between the summoner and their summon(s), the mundane modern world around them, and the strange game they have been pulled into. ` +
